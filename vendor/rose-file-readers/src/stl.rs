    ...
    fn read(
        ...
        let read_stl_entry = |reader: &mut RoseFileReader,
         ...
         -> Result<(u32, u32), anyhow::Error> {
            ...
            // Some server-modified STL files contain text that isn't valid
            // UTF-8 (e.g. leftover EUC-KR bytes or other codepages in a
            // custom item/quest name). Rather than fail to load the entire
            // string table over one bad entry, sanitise lossily.
            let sanitized: std::borrow::Cow<[u8]> = match str::from_utf8(text_bytes) {
                Ok(_) => std::borrow::Cow::Borrowed(text_bytes),
                Err(_) => std::borrow::Cow::Owned(
                    String::from_utf8_lossy(text_bytes).into_owned().into_bytes(),
                ),
            };
            let text_bytes_length = sanitized.len();
